...
//                                     {edit ? (<h1>Edit Project</h1>) : (<h1 style={{color:'red'}}>Create Project</h1>)}
//                                     <button onClick={() => closeModal()} className=' absolute right-6 top-4 text-gray-500 hover:bg-gray-100 rounded focus:outline-none focus:ring focus:ring-offset-1 focus:ring-red-200 '>
...
const AddProjectModal = ({ isModalOpen, closeModal, edit = false, id = null }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!edit) {
            axios.post('http://localhost:9000/project/', { title, description: desc, assignedto })
                .then((res) => {
                    closeModal();
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
            ...
            axios.put(`http://localhost:9000/project/${id}`, { title, description: desc, assignedto })
                .then((res) => {
                    closeModal();
                    const customEvent = new CustomEvent('projectUpdate', { detail: { ...res.data } });
                    document.dispatchEvent(customEvent);
                        ...
                        >
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <form onSubmit={handleSubmit} className='gap-4 px-8 py-4'>
                                    ...
                                    <div className='mb-2'>
                                        <label htmlFor="Description" className='block text-blue-600'>Description</label>
                                        <textarea value={desc} onChange={(e) => setDesc(e.target.value)} className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-red-400' placeholder='Project description'></textarea>
                                    </div>
                                    <div className='mb-2'>
                                        <label htmlFor="AssignedTo" className='block text-blue-600'>Assigned to</label>
                                        <input value={assignedto} onChange={(e) => setAssignedto(e.target.value)} type="text" className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-red-400' placeholder='Project assigned to'/>
                                    </div>
                                    <div className='flex justify-end items-center space-x-2'>
                                        <BtnSecondary onClick={() => closeModal()}>Cancel</BtnSecondary>
                                        <BtnPrimary>Save</BtnPrimary>
