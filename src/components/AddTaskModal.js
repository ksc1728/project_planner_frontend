...
const AddTaskModal = ({ isAddTaskModalOpen, setAddTaskModal, projectId = null, taskId = null, edit = false, refreshData }) => {
        ...
        if (edit && isAddTaskModalOpen) {
            axios.get(`http://localhost:9000/project/${projectId}/task/${taskId}`)
                .then((res) => {
                    ...
                    setAssignedto(res.data[0].task[0].assignedTo);
                })
                .catch((error) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (!edit) {
            axios.post(`http://localhost:9000/project/${projectId}/task`, { title, description: desc, assignedto })
                .then((res) => {
            ...
            axios.put(`http://localhost:9000/project/${projectId}/task/${taskId}`, { title, description: desc, assignedto })
                .then((res) => {
                    setAddTaskModal(false)
                        ...
                        >
                            <Dialog.Panel className="rounded-md bg-white w-6/12">
                                ...
                                <form onSubmit={handleSubmit} className='gap-4 px-8 py-4'>
                                    <div className='mb-3'>
                                        <label htmlFor="title" className='block text-yellow-600'>Title</label>
                                    ...
                                    <div className='mb-2'>
                                        ...
                                        <input value={assignedto} onChange={(e) => setAssignedto(e.target.value)} type="text" className='border border-gray-300 rounded-md w-full text-sm py-2 px-2.5 focus:border-indigo-500 focus:outline-offset-1 focus:outline-indigo-400' placeholder='Project assigned to'/>
                                    </div>
